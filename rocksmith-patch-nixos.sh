...
WINEASIODLLS=(
  "/lib32/wine/i386-unix/wineasio32.dll.so"
...
patch_wineasio_32bit() {
  ...
  safe_copy "${1}" "${PROTONPATH}/${FILES_OR_DIST}/lib/wine/i386-unix/wineasio32.dll.so"
  if [[ $1 == *.so ]]; then
    local wineasio_dll=$(echo ${1} | sed -e 's|/i386-unix/wineasio32.dll.so|/i386-windows/wineasio32.dll|g')
    if [ -e "${1}" ] && [ -e "${wineasio_dll}" ]; then
...
patch_wineasio_64bit() {
  ...
  safe_copy "${1}" "${PROTONPATH}/${FILES_OR_DIST}/lib64/wine/x86_64-unix/wineasio64.dll.so"
  ...
  if [[ $1 == *.so ]]; then
    ...
    local wineasio_dll=$(echo ${1} | sed -e 's|/x86_64-unix/wineasio64.dll.so|/x86_64-windows/wineasio64.dll|g')
    if [ -e "${1}" ] && [ -e "${wineasio_dll}" ]; then
